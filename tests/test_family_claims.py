...
# nowhere else.  Matching every string in the package would be wrong here:
# agentdiff's whole job includes spotting a credential someone left in a diff,
# so its rule table names these patterns on purpose.  Naming a credential and
# reading one are opposite acts, and only the second breaks the claim.
